...
**Status: maintained with the code (Phase 2G). Columns follow spec §19 (data map).**
...
| Workout sessions + definition snapshots | User starting/completing workouts | Local SQLite, `workout_sessions` (incl. `definition_json`, `cursor_json`) — `LOCAL STORAGE` | Record training history, resume sessions | Only if the user exports history — `USER-INITIATED SHARING` | **No** | Yes (CSV / JSON history export) | Full wipe (no per-session delete UI yet) | Until user deletes or wipes | `cursor.timer.expiresAt` is the canonical timer source |
