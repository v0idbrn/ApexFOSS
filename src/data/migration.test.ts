...
/**
 * Migration / data-version sanity for Phase 2E.
 * Goal: prove the installed schema is v3, migrations infrastructure is wired,
 * routine_blocks carries optional interval_json, and readiness_tests exists.
 *
...
describe('migration / data version sanity', () => {
  it('schemaVersion is 3 (Phase 2E readiness tests)', () => {
    expect(schemaVersion).toBe(3);
    expect(schema.version).toBe(3);
  });

  it('all ten release tables are present with expected names', () => {
  ...
  it('migrations infrastructure is validated; v1→v2 and v2→v3 steps', () => {
    expect(migrations.validated).toBe(true);
    expect(migrations.minVersion).toBe(1);
    expect(migrations.maxVersion).toBe(3);
    expect(migrations.sortedMigrations).toHaveLength(2);
